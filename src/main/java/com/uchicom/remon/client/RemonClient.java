 ...
 * (リモートクライアント)-(経由サーバ)-(ローカルクライアント)構成で使用する. クライアントからカラー白黒、キャプチャ間隔、差分ＯＮＯＦＦ、を制御したい
 *
  ...
  public void connect(String hostName, int port) {
    ...
    try {
      ...
      panel.addMouseListener(
          new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent e) {
              write(
                  Constants.COMMAND_MOUSE_RELEASE,
                  modifiersEx
                      & (~e.getModifiersEx())
      ...
      ImageReceiver receiver =
          mono ? new Receiver(socket, panel, aes, iv) : new ImageReceiver(socket, panel);
