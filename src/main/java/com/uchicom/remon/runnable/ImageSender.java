...
public class ImageSender implements Runnable {
  ...
  public void run() {
    ...
    try (GZIPOutputStream gos = new GZIPOutputStream(socket.getOutputStream(), true)) {
      ...
      int maxSize =
          11
              + (screenRect.width * screenRect.height + 7) / 8
              + screenRect.width * screenRect.height * 3;

      int cnt = 0;

      byte[] bytes = new byte[maxSize];
      while (!socket.isOutputShutdown()) {
        if (Constants.DEBUG) printLog("start", base);
        if (Constants.DEBUG) System.out.println("送信" + cnt++);
        ...
        if (Constants.DEBUG) printLog("getPixels", base);
        // ここでイメージを2回チェックしてるから遅いんだな。
        //				if (!Arrays.equals(previousImage, image[index])) {//5msec
        if (Constants.DEBUG) printLog("Arrays.equals", base);
        ...
        if (Constants.DEBUG) printLog("flush", base);
  ...
  /**
   * x,y座標の検知方法についての考察 0,0→1,0でyの探査、yが見つかったら、0,y+1→x-1,y+1→0,y+2と探査して最初に現れたyの地点よりも小さなxを探索する
